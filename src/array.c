...
int akit_array_grow(AkitArray* array, int64_t n) {
  if (array->item_size <= 0 || n <= 0) return 0;
  array->avail += akit_array_get_capacity(array) * n;
  array->items = realloc(array->items, (array->length+array->avail) * array->item_size);
...
void akit_array_clear(AkitArray *array) {
  if (!array) RETURN_UNLOCK();
  // pthread_mutex_trylock(&array->lock);
  if (array->length <= 0) RETURN_UNLOCK();
  if (array->items == 0) RETURN_UNLOCK();
...
void akit_array_remove(AkitArray *array, void *element,
  ...
  if (array->item_size == 0) return;
  if (array->items == 0) return;
  if (array->item_size != sizeof(element)) return;
  ...
  if (free_method && free_method != (void *)0) free_method(array->items[index]);
...
void akit_array_concat(AkitArray *a, AkitArray *b) {
  if (!b) return;
  if (akit_array_is_empty(b)) return;
  if (b->length >= a->avail) {
...
unsigned int akit_array_is_empty(AkitArray *array) {
  if (!array) return 1;
  if (array->items == 0) return 1;
  if (array->length <= 0) return 1;
  if (array->item_size <= 0) return 1;
...
unsigned int akit_array_includes(AkitArray *array, void *item) {
  ...
  for (uint32_t i = 0; i < array->length; i++) {
    if (!array->items) continue;
    if (array->items[i] == item) return 1;
  }
...
loop_start:
  for (int64_t i = 0; i < array->length; i++) {
    ...
    if (akit_array_count(array, array->selector ? array->selector(item) : item) >
        1) {
...
int akit_array_copy(AkitArray *dest, AkitArray src) {
  ...
  if (!src.items) return 0;
  if (akit_array_is_empty(&src)) return 0;
  ...
  if (!dest->length || !dest->item_size) return 0;
