...
void akit_engine_process_clip(AkitEngine *engine, AkitSoundClip *clip,
  ...
  if (clip->sound.gain <= 0.000001f) {
    clip->finished = true;
    return;
  }
  akit_dsp_process_fx(engine, clip);
  ...
  if (clip->cursor >= (clip_length / sizeof(float))) {
    clip->finished = true;
    return;
  }
  ...
  clip->time = (1.0f / sample_rate) * ((double)MAX(0, clip->cursor - clip->sound.cursor_start) / (double)divisor);
}

void akit_engine_process(AkitEngine *engine, double time) {

  int64_t channels = akit_engine_get_channels(engine);
  int64_t frame_length = akit_engine_get_frame_length(engine);
  float sample_rate = akit_engine_get_sample_rate(engine);

  if (!engine->mixed_clip) {
    engine->mixed_clip = NEW(AkitSoundClip);
    engine->mixed_clip->sound.data = (float *)calloc(frame_length*4, sizeof(float));
  } else if (engine->mixed_clip->sound.data != 0) {
    memset(&engine->mixed_clip->sound.data[0], 0, (frame_length*4)*sizeof(float));
  }

  AkitSoundClip *mixed_clip = engine->mixed_clip;

  mixed_clip->time = 0;
  mixed_clip->sound.channels = 2;
  mixed_clip->sound.gain = 1.0f;
  mixed_clip->sound.duration = 1.0f;
  mixed_clip->sound.length = frame_length;
  mixed_clip->sound.sample_rate = sample_rate;
  mixed_clip->finished = false;
  ...
  for (int64_t i = 0; i < engine->clips.length; i++) {
    ...
    if (clip->finished && clip->cursor > 0) {
      ...
      akit_engine_process_clip(engine, clip, mixed_clip->sound.data,
                               mixed_clip->sound.length * channels, time);
    }
  }

  engine->mixed_clip = mixed_clip;
}
...
void *akit_engine_thread(void *ptr) {
  ...
  while (true) {
    ...
    pthread_mutex_lock(&engine->push_lock);
    akit_engine_process(engine, engine->time);
    pthread_mutex_unlock(&engine->push_lock);


    AkitSoundClip *clip = engine->mixed_clip;

    if (!clip)
      continue;

    akit_driver_buffer_data(&engine->driver, clip->sound.data,
                            clip->sound.length * channels);

    akit_msleep(time_unit * 1000.0f * channels);
  //  akit_array_remove(&engine->queue, clip,
    //                  (AkitArrayFreeFunction)akit_sound_clip_free);
